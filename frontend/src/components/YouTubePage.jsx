  ...
  const loadLiveChannels = async () => {
    try {
      const data = await fetchYT("search", {
        eventType: "live", type: "video", q: "news live stream free",
  ...
  return (
    <div className="min-h-screen bg-black py-8 px-4">
      <div className="max-w-7xl mx-auto">
        ...
        {activeTab === "live" && (
          <div>
            <h2 className="text-white font-bold text-xl mb-4">?? Free Live Channels</h2>
            <div className="flex gap-4 overflow-x-auto pb-4 scrollbar-hide mb-8">
              {liveChannels.map(v => <VideoCard key={v.id?.videoId} video={v} onPlay={playVideo} />)}
            </div>
          </div>
        )}
      </div>
