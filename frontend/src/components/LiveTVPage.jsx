...
import { ArrowLeft, Tv, ChevronRight, ChevronDown, Menu, X } from 'lucide-react';
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', background: '#0a0a0a', color: '#fff', fontFamily: 'sans-serif', overflow: 'hidden' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 16px', borderBottom: '1px solid #222', background: '#111', flexShrink: 0, zIndex: 10 }}>
        ...
        <button
          ...
          style={{ display: 'flex', alignItems: 'center', gap: 5, background: sidebarOpen ? '#1f2937' : '#1a1a1a', border: '1px solid #333', borderRadius: 8, color: '#fff', cursor: 'pointer', fontSize: 12, padding: '6px 10px', flexShrink: 0 }}
        >
      ...
      <div style={{ display: 'flex', flex: 1, overflow: 'hidden', position: 'relative' }}>
        ...
        {sidebarOpen && (
          <div style={{ position: isMobile ? 'absolute' : 'relative', top: 0, left: 0, height: '100%', width: 260, background: '#111', borderRight: '1px solid #222', overflowY: 'auto', flexShrink: 0, zIndex: 30 }}>
            <div style={{ padding: '10px 14px', borderBottom: '1px solid #1e1e1e', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <span style={{ color: '#888', fontSize: 11, textTransform: 'uppercase', letterSpacing: '0.08em', fontWeight: 700 }}>Channels</span>
              <button onClick={() => setSidebarOpen(false)} style={{ background: 'none', border: 'none', color: '#555', cursor: 'pointer', padding: 2 }}><X size={14} /></button>
            </div>
            {genres.map(genre => {
              ...
              return (
                <div key={genre}>
                  <button
                    ...
                    style={{ width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '11px 16px', background: isOpen ? '#1a1a1a' : 'transparent', border: 'none', borderBottom: '1px solid #1e1e1e', color: isOpen ? '#fff' : '#999', cursor: 'pointer', fontSize: 12, fontWeight: 700, textAlign: 'left', textTransform: 'uppercase', letterSpacing: '0.06em' }}
                  >
        ...
        <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#000', position: 'relative', minWidth: 0 }}>
          {!activeChannel && (
            <div style={{ textAlign: 'center', padding: 20 }}>
              ...
              <p style={{ fontSize: 12, color: '#333', marginTop: 6 }}>Tap the menu button above to browse channels</p>
            </div>
          )}
          {activeChannel && (
