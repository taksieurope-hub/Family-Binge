...
export const saveToWatchHistory = (content, season = 1, episode = 1, progress = 0) => {
  try {
    ...
    const uid = localStorage.getItem('fb_uid');
    if (uid) {
      fetch('https://api.familybinge.com/api/auth/watch-history/' + uid, {
        method: 'POST',
...
const VIDEO_SOURCES = [
  { name: 'AutoEmbed', getUrl: (type, id, s, e) => type === 'series' ? `https://autoembed.co/tv/tmdb/${id}-${s}-${e}` : `https://autoembed.co/movie/tmdb/${id}` },
...
const ContentDetailModal = ({ content, onClose, onPlayVideo, accessStatus, onExpiredClick }) => {
  ...
  const handleWatchNow = () => {
    ...
    if (details) {
      ...
      fetch('https://api.familybinge.com/api/auth/analytics/play', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          session_id: sid,
    ...
    return (
      <div ref={playerContainerRef} className="fixed inset-0 z-[100] bg-black flex flex-col">
        ...
        <div className="flex-1 bg-black relative">
          ...
          {getStreamUrl() && (
            <iframe sandbox="allow-scripts allow-same-origin allow-forms allow-presentation allow-pointer-lock allow-popups"
              ref={iframeRef}
