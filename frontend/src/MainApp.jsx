...
function MainApp() {
    ...
    const checkAuth = async () => {
      ...
      try {
        ...
        for (let attempt = 0; attempt < 3; attempt++) {
          res = await fetch(`https://api.familybinge.com/api/auth/me/${uid}`);
          if (res.ok) break;
        ...
        if (res.ok) {
          ...
          await fetch(`https://api.familybinge.com/api/payment/user/${uid}`, {
  method: 'POST',
  ...
  useEffect(() => {
    ...
    if (!sessionStorage.getItem('fb_visit_tracked')) {
      ...
      fetch('https://api.familybinge.com/api/auth/analytics/visit', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
